...
DEFAULT_SERVICES=""   # e.g. 'user:comfyui.service,user:comfyui-dashboard.service'
...
Options:
  --reinstall           Uninstall then install (default)
  --install-only        Install only, fail if existing installation detected
  --uninstall           Uninstall only, then exit
  --no-start            Do not enable or start the service after install
  --port N              Port to bind (default: $DEFAULT_PORT)
  --bind HOST           Bind host (default: $DEFAULT_BIND)
  --services LIST       Comma-separated services to monitor, e.g. "user:comfyui.service,user:comfyui-dashboard.service"
  -h, --help            Show this help

Examples:
  $(basename "$0")                              # Reinstall and start on 127.0.0.1:8080
  $(basename "$0") --install-only --port 8081   # Install only, custom port
  $(basename "$0") --uninstall                  # Uninstall everything (safe defaults)
EOF
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    --reinstall)      REINSTALL="true"; INSTALL_ONLY="false"; UNINSTALL_ONLY="false"; shift;;
    --install-only)   INSTALL_ONLY="true"; REINSTALL="false"; UNINSTALL_ONLY="false"; shift;;
    --uninstall)      UNINSTALL_ONLY="true"; REINSTALL="false"; INSTALL_ONLY="false"; shift;;
...
EOF
  ...
  # Generate ACTION_TOKEN and SECRET_KEY if empty
  gen_hex() {
  ...
  if ! grep -q "^ACTION_TOKEN=" "$APP_DIR/.env"; then
    echo "ACTION_TOKEN=$(gen_hex)" >> "$APP_DIR/.env"
  else
    if [[ -z "$(grep '^ACTION_TOKEN=' "$APP_DIR/.env" | cut -d= -f2-)" ]]; then
      sed -i "s/^ACTION_TOKEN=.*/ACTION_TOKEN=$(gen_hex)/" "$APP_DIR/.env"
    fi
  fi
...
pkgs = ["flask", "gunicorn", "python-dotenv"]
cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "pip"] + pkgs
...
post_install_notes() {
  ...
  echo "Bind:        $BIND_HOST"
  echo "Port:        $PORT"
  echo "Dashboard:   http://$BIND_HOST:$PORT (local only unless you change bind)"
  echo ""
  echo "Manage service:"
...
if [[ "$REINSTALL" == "true" ]]; then
  ...
  # basic guardrails
  if [[ -d "$APP_DIR" ]]; then die "App directory exists ($APP_DIR). Use --reinstall to replace."; fi
  if conda env list | grep -qE "^\s*$ENV_NAME\s"; then die "Conda env exists ($ENV_NAME). Use --reinstall to replace."; fi
  if [[ -f "$UNIT_PATH" ]]; then die "Unit file exists ($UNIT_PATH). Use --reinstall to replace."; fi
fi
